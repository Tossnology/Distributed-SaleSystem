...
function loadWarehourseOrderList(worderList) {
    ...
    for (var i = 0; i < worderList.length; i++) {
        ...
        //添加详情按钮
        else if (worderList[i].status == "2" && worderList[i].type.toString() == "2" && worderList[i].principalid.toString() == getCookie("warehourseid")) {
        ...
        //添加详情，审核
        else if (worderList[i].status == "2" && worderList[i].type.toString() == "2" && worderList[i].principalid.toString() != getCookie("principalid")) {
            var detailButton = document.createElement("button");
            detailButton.type = "button";
            detailButton.id = "detail-btn";
            detailButton.setAttribute("value", worderList[i].id); //将货品id封装在value中
        ...
        //审核中且源地址为外部进销商
        //添加详情，审核
        else if (worderList[i].status == "2" && worderList[i].type.toString() == "1") {
            var detailButton = document.createElement("button");
            detailButton.type = "button";
            detailButton.id = "detail-btn";
            detailButton.setAttribute("value", worderList[i].id); //将货品id封装在value中
